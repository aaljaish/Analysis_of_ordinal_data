/*                          INSTRUCTIONS FOR USE 
   ...
   SOURCE: https://support.sas.com/kb/37/addl/fusion_37944_21_empiricallogitplot.sas.txt
...
/*--------------------------------------------------------------------------
  Ordinal_Cluster_Analysis Macro
...
%Macro Ordinal_Cluster_Analysis(data=, y=, group=, cluster_id=);
	...
	Title "Visiual Check for the assumption of proportional odds";
	%EmpiricalLogitPlot(data=&data, y=&y, x=&group, options= nopanel)

	Title "Results for a Proportional Odds Model";
	proc glimmix data=&data method=laplace;
		class &group &cluster_id;
		model &y = &group / alpha=0.05 dist=multinomial link=cumlogit solution oddsratio;  /* 'solution' and 'oddsratio' options added */
		random intercept / subject=&cluster_id solution;  /* Add random intercept for the cluster */
		ods output OddsRatios=OR;  /* Save the odds ratio estimates to a dataset */
	run;

	title "Print the odds ratio estimates";

	proc print data=OR;  /* Print the odds ratio estimates */
	run;

	title;

	/* Create a dataset for converting odds ratio to cumulative probability */
	data Probabilities;
		set OR;

		/* Convert odds ratio to cumulative probability using the formula:
		Probability = OddsRatio / (1 + OddsRatio) */
		CumulativeProb = Estimate / (1 + Estimate);

		/* Keep only the relevant variables for the results */
		keep &group Estimate CumulativeProb;
	run;

	/* Print the cumulative probabilities */
	Title "Print the cumulative probabilities";
	proc print data=Probabilities;
		var &group Estimate CumulativeProb;
	run;
